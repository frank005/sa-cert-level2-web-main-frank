...
async function join() {
  ...
  if (currentStream == "camera") {
  ...
  } else if (currentStream == "catinhat") {
    //copy code from below, should be the similar
    //https://stackoverflow.com/questions/15440404/html5-canvas-and-javascript-document-getelementbyid-returns-null
    var capstr = document.getElementById("myCanvas").captureStream();
    ...
    [ options.uid, localTracks.videoTrack ] = await Promise.all([
      ...
      client.join(options.appid, options.channel, options.token || null, options.uid || null),
      // Create tracks to the localcamera.
      AgoraRTC.createCustomVideoTrack({mediaStreamTrack:capstr.getVideoTracks()[0]})
    ]);
